...
import { Map, Calendar, Wallet, Sparkles, Loader2, ArrowRight, ShoppingBag, MapPin, X } from 'lucide-react';
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message?.includes('API_KEY') || err.message?.includes('401')) {
        setError('AI Planner belum dikonfigurasi. Pastikan GEMINI_API_KEY sudah diisi di file .env');
            ...
            <div className="bg-white dark:bg-stone-900 p-8 md:p-12 rounded-[40px] shadow-xl border border-stone-100 dark:border-stone-800 min-h-[600px]">
              {isLoading ? (
                ...
                <div className="h-full flex flex-col items-center justify-center py-32 text-center">
                  ...
                  <p className="text-stone-500 dark:text-stone-400 max-w-sm">{error}</p>
                  <button
                    onClick={() => setError('')}
                ...
                >
                  ...
                  <div className="mt-16 pt-12 border-t border-stone-100 dark:border-stone-800 flex flex-col sm:flex-row gap-4 justify-between items-center">
                    <button
                      onClick={() => { setItinerary(''); setRecommendedProducts([]); }}
